...
先读取 `.github/skills/_shared/index.md`：
...
- 需要判断能力应属于 framework core、installed adapter instance、local override、产品层、代码层或 Architecture/Proposal/Experience。
...
1. 读取本 skill、`_shared/index.md`、index 选中的组件、相关 instructions、owning skills、目标文件、相邻模块、测试和调用方。
...
- 项目专用归属和 reference/adoption 约束按 `.ousia/workflow.json` 和 installed `.ousia/**` design facts 判断。
...
项目专用偏好、经验和约束归 installed `.ousia/**` design primitives；查证路线和 review attacks 进 Experience。
...
- 推荐方案和取舍理由。
- 推荐方案如何改善边界，而不是只增加层数。
- 第一个可实施的纵向切片：目标语义、跨越 owner、边界 API、实现文件、design facts、测试层级、完成条件、排除范围，以及哪些边界调整是必要前置。
