...
这个文件只做入口 skill 的通用路由表。规范内容由 `.github/instructions/**` 提供；项目语义、项目证据和 Ousia-specific checklist 归 `design/**` owning docs，不作为 skills 扩展层维护。
...
- 需要项目证据、领域 review attacks 或 Ousia-specific checklist 时，由入口 skill 按当前任务 scope 读取 `design/**` 中的 owning docs 或 evidence docs；本 shared index 不维护项目扩展路由。
