...
这些规则用于设计、review 和演进 Ousia Workflow 的代码、文档、instructions、skills、shared assets、profile skeleton、validation policy 和 workflow。它只规定 prompt 系统也要遵守的通用架构原则；当前 ownership 和 upgrade boundary 由 [.ousia/workflow.json](../../.ousia/workflow.json) 与 [.ousia/design/execution/ousia-workflow-extraction/index.md](../../.ousia/design/execution/ousia-workflow-extraction/index.md) 维护。
...
- 命名前缀：workflow 项目命名为 `ousia`。通用 workflow 实现、通用 instruction、通用 skill、通用 mode 和通用 validation 能力使用 `ousia-` 前缀；profile-specific、adapter payload 和 project binding 投影使用 `ext-<profile>-` 或 manifest 声明的 profile 前缀。`harness` 只指外部 agent runtime 或 execution carrier。
...
- Project surface：未来 `ousia workflow install` 会生成 `.ousia/**` 作为 Ousia-defined project surface，用于 profile manifest、project design layout、research/review routing 和 pending area。项目只在 Ousia-defined slots 中填写事实。
...
- `.ousia/**` 是 Ousia-defined project surface；`.ousia/design/**` 是其中的项目设计布局。产品文档、implementation docs、proposal 和 design-owned evidence 要各有 owner，稳定结论进入对应 design area，快速查证经验进入 Research area。Workflow policy 和 upgrade ownership 由 Ousia Workflow 拥有。
- 测试和验证是闭环的一部分，不是实现后的装饰；每个非平凡变更都应说明验证覆盖了什么风险。

## 职责分层

- 硬规范、入口界面、任务模式、project design areas、profile payload 和验证规则必须有不同 owner。
...
- Workflow 只承载闭环编排、验证选择、upgrade ownership 和执行载体边界；某个入口的 prompt 内容、输出协议和 handoff packet 归对应入口。
- 当前文件路径到这些 owner 的映射由 [.ousia/workflow.json](../../.ousia/workflow.json) 和 workflow extraction plan 声明，不在本 instruction 中重复维护完整清单。
...
- 新文件名是否遵守 `ousia-` / manifest profile 前缀；如果不能判断归属，是否应先进入暂存区而不是发明第三套命名？
...
- 如果用户指出的是领域 baseline、reference 对齐或 review 攻击面缺失，应同步 profile payload、经验/证据库、review attacks 和 owning design docs；不要把领域长期目标偷放进通用入口 instruction 或 facade skill。
- 能通过调整现有层解决时，不新增层。
