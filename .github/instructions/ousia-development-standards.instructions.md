...
- 如果任务涉及 kernel、OSTD、tooling、Cargo target 或 implementation design，还必须读取 `.github/instructions/ext-ousia-kernel-boundaries.instructions.md`。
...
- 如果任务涉及验证、review 闭环、subagent 使用边界、handoff 或最终报告，还必须读取 `.github/instructions/ext-ousia-workflow.instructions.md`。

## 组合式工作流

Skills 只负责少量外部维度和入口输出协议：`architecture-planner` 使用 `mode` 与 `target`，`black-team-review` 使用 `subject` 与 `mode`。组合方式由 `.github/instructions/ext-ousia-workflow.instructions.md`、`.github/instructions/ousia-prompt-architecture.instructions.md` 和入口 skill 的“组合资产”段约束；这些 prompt 结构也必须服从项目元架构规则。
