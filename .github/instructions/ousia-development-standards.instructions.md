...
- 架构师读取全部开发规范模块，并按范围追加 documentation、workflow、language plugin 或 installed adapter 声明的项目规则。
...
- 如果任务涉及当前 installed adapter 声明的领域边界、验证矩阵或特殊规范，还必须读取 `.ousia/**` 路由到的 corresponding instruction。
...
不要在 skill 中复制整份规范。具体产品层设计、代码实现、project 边界、文档归属和测试质量规则由 instructions 或 installed `.ousia/**` adapter instance 提供；shared assets 只描述任务形状。
